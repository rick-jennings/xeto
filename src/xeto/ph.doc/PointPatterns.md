...
## Limit Setpoints
Limits are modeled with the following subtypes of
[ph.points::LimitSp]:

  - [ph.points::MinSp]: setpoint for the lower limit that a control
    system keeps a value at or above, such as the minimum airflow of
    a VAV
  - [ph.points::MaxSp]: setpoint for the upper limit that a control
    system keeps a value at or below, such as an electrical demand
    limit

These roles are reflected in setpoint spec names: names ending in
"MinSp" or "MaxSp" denote a [ph.points::MinSp] or [ph.points::MaxSp].

## Target Setpoints
The same value may have several setpoints, each applying only under
certain conditions such as occupancy, heating/cooling mode, or a
demand response event.  A value may have many candidate setpoints
but at most one effective setpoint, which resolves the conditions
currently in effect.  Candidate setpoints are configured values
that may be written by operators or supervisory systems, while the
effective setpoint is reported by the control system as read-only
feedback of the setpoint it resolved.  The effective setpoint is
the authoritative record of the value control is driving toward,
available without re-deriving the conditions and prioritization
that selected it.  Comparing it to the sensed value shows whether
control is meeting its target.

To distinguish the effective setpoint driving control from
candidate setpoints, the following subtypes of
[ph.points::TargetSp] are used:

  - [ph.points::CandidateSp]: setpoint the control loop may use
    while its conditions are met
  - [ph.points::EffectiveSp]: setpoint the control loop is currently
    using

These roles are reflected in setpoint spec names: names ending in
"EffectiveSp" or "CandidateSp" denote an [ph.points::EffectiveSp]
or [ph.points::CandidateSp].  Every [ph.points::EffectiveSp]
subtype includes "Effective" in its name, while
[ph.points::CandidateSp] subtypes may instead be named by their
condition, such as "Occ" in [ph.points::ZoneAirTempOccHeatingSp].
...
The candidate and effective setpoint pattern is also specialized
for heating and cooling modes.  For example, zone air temperature
defines per-mode effective setpoints,
[ph.points::ZoneAirTempHeatingEffectiveSp] and
[ph.points::ZoneAirTempCoolingEffectiveSp], each resolving the
candidate setpoints of its mode.  [ph.points::ZoneAirTempEffectiveSp]
in turn resolves the heating and cooling modes when present.
